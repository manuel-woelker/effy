use effy_base::error::err;
use effy_base::error::{Context, EffyResult};
use effy_pal::{FileChangeCallback, FileChangeEvent, FilePath, Pal};
...
impl PalReal {
    ...
    fn resolve_path(&self, path: &FilePath) -> EffyResult<PathBuf> {
        Ok(path.to_path(&self.base_path))
    }

    fn relativize_path(&self, path: &Path) -> EffyResult<FilePath> {
